...
def scrape_apartment(apartment_name, url, page):
    ...
    for idx, raw_card in enumerate(cards):
        try:
            ...
            floorplan = children[0].inner_text().strip()
            sqft = children[2].inner_text().strip()
            move_in = children[3].inner_text().strip()
            price = children[4].inner_text().strip()
            ...
            print(
                f"[WARN] Skipped unit {idx+1} from {apartment_name} due to error: {e}"
            )
            continue
